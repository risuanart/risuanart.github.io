/* tw-postal-codes.js —— 台灣 22 縣市／371 個鄉鎮市區的 3 碼郵遞區號對照表，
   給購物車頁「縣市／鄉鎮市區」下拉選單用（見 js/checkout.js）：選了鄉鎮
   市區，郵遞區號直接查表帶出來，不用讓客人自己猜／打錯。資料整理自中華
   郵政公告的 3 碼郵遞區號，新竹市／嘉義市這種全市單一郵遞區號的情況，
   底下每個行政區會查到同一組號碼（例如新竹市東區／北區／香山區都是
   300），這是正確的，不是重複打錯。

   純資料檔，不做任何 DOM 操作，其他頁面用不到郵遞區號查詢就不用載入。 */
window.TW_POSTAL_CODES = {
